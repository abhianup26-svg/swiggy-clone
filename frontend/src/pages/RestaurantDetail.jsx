...
export default function RestaurantDetail() {
  ...
  if (loading)      return <div style={styles.center}><p>Loading menu...</p></div>;
  if (!restaurant)  return <div style={styles.center}><p>Restaurant not found.</p></div>;
          ...
          <div key={category} style={styles.categoryBlock}>
            ...
            {restaurant.menu.filter(item => item.category === category).map(item => (
              <div key={item._id} style={styles.menuItem}>
                <span style={{ ...styles.vegBadge, borderColor: item.isVeg ? '#0f8a65' : '#e43b3b' }}>
                  <span style={{ ...styles.vegDot, background: item.isVeg ? '#0f8a65' : '#e43b3b' }} />
                </span>
                ...
                <div style={styles.addSection}>
                  {getQty(item._id) === 0 ? (
                    ...
                    <div style={styles.qtyControl}>
                      ...
                      <span style={styles.qtyNum}>{getQty(item._id)}</span>
...
const styles = {
  ...
  header:   { background: '#fff', padding: '24px 32px', borderBottom: '1px solid #eee' },
  ...
  categoryTitle: { fontSize: '16px', fontWeight: '700', color: '#333', marginBottom: '16px', paddingBottom: '8px', borderBottom: '1px solid #eee' },
  menuItem: {
    background: '#fff', borderRadius: '12px', padding: '16px',
    marginBottom: '12px', display: 'flex', alignItems: 'center',
    gap: '12px', boxShadow: '0 1px 6px rgba(0,0,0,0.06)',
  },
  vegBadge: { width: 18, height: 18, border: '2px solid', borderRadius: '3px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  vegDot:   { width: 8, height: 8, borderRadius: '50%' },
  itemInfo:  { flex: 1 },
  ...
  qtyControl: {
    ...
    background: '#fc8019', borderRadius: '8px', padding: '6px 12px',
  },
  ...
  cartBar: {
    ...
    transform: 'translateX(-50%)', background: '#fc8019',
    borderRadius: '16px', padding: '16px 24px',
    display: 'flex', alignItems: 'center',
    justifyContent: 'space-between', width: '90%',
    maxWidth: '500px', boxShadow: '0 8px 24px rgba(252,128,25,0.4)',
  },
